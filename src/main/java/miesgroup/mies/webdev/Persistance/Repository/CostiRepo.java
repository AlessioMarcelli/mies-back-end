...
public class CostiRepo {
    ...
    public void aggiungiCostoTrimestrale(Costi costo) {
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO dettaglio_costo (Descrizione, Categoria, Unità_Misura, Trimestrale, Costo) VALUES (?, ?, ?, ?, ?)", PreparedStatement.RETURN_GENERATED_KEYS);) {
    ...
    public void aggiungiCostoAnnuale(Costi costo) {
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO dettaglio_costo (Descrizione, Categoria, Unità_Misura, Annuale, Costo) VALUES (?, ?, ?, ?, ?)", PreparedStatement.RETURN_GENERATED_KEYS);) {
                statement.setString(1, costo.getDescrizione());
                statement.setString(2, costo.getCategoria());
                statement.setString(3, costo.getUnitaMisura());
    ...
    public ArrayList<Costi> getAllCosti() {
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT Descrizione, Unità_Misura, Trimestrale, Annuale, Costo, Categoria FROM dettaglio_costo");) {
